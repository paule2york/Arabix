...
const shopCardHtml = (product, index, compact = false) => {
  ...
  const badge = textFrom(product.badge) || (langKey === "ar" ? "\u0642\u0627\u0644\u0628 \u062c\u0627\u0647\u0632" : "Ready Theme");
...
const renderShopCategoryPage = async (root) => {
  ...
  const category = shopCategoryById(data, catId) || (data.categories || [])[1] || { id: "all", en: "Themes", ar: "\u0627\u0644\u0642\u0648\u0627\u0644\u0628", icon: "market" };
  ...
  hero.innerHTML = '<div class="shop-category-title"><span>' + shopIconSvg(category.icon) + '</span><p class="eyebrow">' + (langKey === "ar" ? "\u0641\u0626\u0629 \u0627\u0644\u0642\u0648\u0627\u0644\u0628" : "Theme category") + '</p><h1>' + escapeHtml(textFrom(category)) + '</h1><p>' + escapeHtml(textFrom(category.description)) + '</p></div>';
  list.innerHTML = '<div class="shop-product-row"><div class="shop-row-head"><div><p class="eyebrow">' + (langKey === "ar" ? "\u0643\u0644 \u0627\u0644\u0642\u0648\u0627\u0644\u0628" : "All themes") + '</p><h2>' + escapeHtml(textFrom(category)) + '</h2></div><a href="./shop.html">' + (langKey === "ar" ? "\u0643\u0644 \u0627\u0644\u0641\u0626\u0627\u062a" : "All categories") + '</a></div><div class="shop-grid">' + products.map((product, index) => shopCardHtml(product, index)).join("") + '</div></div>';
...
const renderThemeDetailPage = async (root) => {
  ...
  const addons = Array.isArray(product.addons) ? product.addons : [];
  const productTitle = textFrom(product.title);
  target.innerHTML = '<div class="shop-detail-layout shop-detail-page-layout"><div class="shop-detail-preview"><nav class="shop-breadcrumb"><a href="./shop.html">' + (langKey === "ar" ? "\u0627\u0644\u0645\u062a\u062c\u0631" : "Shop") + '</a><span>/</span><a href="' + shopCategoryHref(product.category) + '">' + escapeHtml(textFrom(category) || product.category) + '</a></nav><div class="shop-detail-topline"><span>' + escapeHtml(textFrom(product.platform) || "Arabix Theme") + '</span><a href="' + escapeHtml(product.liveDemo || "#") + '">' + (langKey === "ar" ? "\u0639\u0631\u0636 \u0645\u0628\u0627\u0634\u0631" : "Live Demo") + '</a></div><h1>' + escapeHtml(productTitle) + '</h1><p>' + escapeHtml(textFrom(product.description)) + '</p><div class="shop-detail-media-wrap">' + buildCmsMedia(product, "shop") + '<b>' + escapeHtml(textFrom(product.badge) || (langKey === "ar" ? "\u0642\u0627\u0644\u0628 \u062c\u0627\u0647\u0632" : "Ready Theme")) + '</b></div></div><aside class="shop-detail-buy"><p class="shop-detail-kicker">' + (langKey === "ar" ? "\u0627\u062e\u062a\u0631 \u0627\u0644\u0631\u062e\u0635\u0629" : "Choose a license") + '</p><div class="shop-license-list"><label><input type="radio" name="shop-license" value="personal" checked data-license-price="' + personal.price + '"><span><strong>' + escapeHtml(textFrom(personal)) + '</strong><em>' + (personal.oldPrice ? '<del>' + formatSar(personal.oldPrice) + '</del>' : '') + '<b>' + formatSar(personal.price) + '</b></em></span></label><label><input type="radio" name="shop-license" value="commercial" data-license-price="' + commercial.price + '"><span><strong>' + escapeHtml(textFrom(commercial)) + '</strong><em>' + (commercial.oldPrice ? '<del>' + formatSar(commercial.oldPrice) + '</del>' : '') + '<b>' + formatSar(commercial.price) + '</b></em></span></label></div><p class="shop-detail-kicker">' + (langKey === "ar" ? "\u062e\u062f\u0645\u0627\u062a \u0625\u0636\u0627\u0641\u064a\u0629" : "Popular services") + '</p><div class="shop-addon-list">' + addons.map((addon) => '<label><input type="checkbox" data-addon-price="' + Number(addon.price || 0) + '" ' + (addon.selected ? 'checked' : '') + '><span><strong>' + escapeHtml(textFrom(addon)) + '</strong><em>+' + formatSar(addon.price || 0) + '</em></span></label>').join("") + '</div><div class="shop-total-row"><span>' + (langKey === "ar" ? "\u0627\u0644\u0625\u062c\u0645\u0627\u0644\u064a" : "Total") + '</span><strong data-shop-total></strong></div><button class="shop-cart-button" type="button" data-shop-order>' + (langKey === "ar" ? "\u0623\u0636\u0641 \u0625\u0644\u0649 \u0627\u0644\u0633\u0644\u0629" : "Add to Cart") + '</button><p class="shop-manual-note">' + (langKey === "ar" ? "\u0628\u0639\u062f \u0627\u0644\u0637\u0644\u0628 \u0633\u0646\u0631\u0633\u0644 \u0644\u0643 \u0631\u0627\u0628\u0637 \u0627\u0644\u062f\u0641\u0639 \u0623\u0648 \u0641\u0627\u062a\u0648\u0631\u0629 Payoneer." : "After the order, we will send a Payoneer invoice or payment link.") + '</p></aside></div>';
  target.querySelectorAll("img").forEach(setupImageFallback);
