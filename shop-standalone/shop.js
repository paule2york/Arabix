(function clientCode(){
  ...
  const demoHref = (p) => String(p.demoUrl || '').trim() || ('./demo.html?slug=' + p.slug);
  const money = (amount) => {
  ...
  function renderDemo(){
    ...
    root.innerHTML = `<header class="preview-bar"><a class="preview-brand" href="./index.html" aria-label="Arabix shop"><img src="./assets/logo-white.png" alt="Arabix" /></a><div class="preview-meta"><strong>${productTitle(p)}</strong><span>${t('demoTag')}</span></div><nav class="preview-actions" aria-label="Preview actions"><a class="preview-details" href="./theme.html?slug=${p.slug}">${t('details')}</a><a class="preview-buy" href="./theme.html?slug=${p.slug}">${t('buyNow')}</a></nav></header><section class="demo-stage"><div class="demo-site"><nav class="demo-nav"><b>${productTitle(p).split(' ')[0]}</b><span>Home</span><span>Services</span><span>Work</span><span>Contact</span></nav><main class="demo-hero"><div><em>${catName(c.id)}</em><h1>${t('demoHero').replace('{category}', catName(c.id).toLowerCase())}</h1><p>${productDetails(p)}</p><a href="./theme.html?slug=${p.slug}">${t('requestTemplate')}</a></div><aside><i></i><b></b><span></span><span></span><div><small></small><small></small><small></small></div></aside></main><section class="demo-grid">${p.features.slice(0,3).map((f,i) => `<article><small>0${i+1}</small><h2>${currentLang === 'ar' ? 'قسم جاهز' : f}</h2><p>${t('builtClear')}</p></article>`).join('')}</section><section class="demo-band"><h2>${t('demoBandTitle')}</h2><p>${t('demoBandCopy')}</p></section></div></section>`;
  }
